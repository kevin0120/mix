...
class MrpProduction(models.Model):
    ...
    quality_alert_ids = fields.One2many('sa.quality.alert', "production_id", string="Alerts")
    ...
    def action_confirm(self):
        for production in self:
            points = self.env['sa.quality.point'].search(production._get_quality_point_domain())
            for point in points:
                if point.check_execute_now():
                    self.env['sa.quality.check'].create(production._get_quality_check_values(point))
