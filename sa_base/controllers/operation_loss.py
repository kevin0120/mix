...
class OperationLoss(http.Controller):
    @http.route('/api/v1/mrp.workcenter.productivity.loss', type='http', methods=['GET', 'OPTIONS'], auth='none', cors='*', csrf=False)
    def _get_production_loss(self, **kw):
        ...
        if 'limit' in kw.keys():
            ...
            limit = DEFAULT_LIMIT
        if 'lossType' not in kw or len(kw['lossType'].split(',')) != 1 or kw['lossType'] not in ['availability','performance','quality','productive']:
