...
// PrintConfigCommand represents the command executed by "rushd config".
...
func (cmd *PrintConfigCommand) Run(args ...string) error {
	...
	if err := config.Validate(); err != nil {
		return fmt.Errorf("%s. To generate a valid configuration file run `rushd config > rush.yaml`.", err)
	}
...
//   3. The first non empty rush.conf file in the path:
//        - ~/.rush/
//        - /etc/rush/
func FindConfigPath(configPath string) string {
	if configPath != "" {
		...
		return configPath
	} else if envVar := os.Getenv("Rush_CONFIG_PATH"); envVar != "" {
		return envVar
	}

	for _, path := range []string{
		os.ExpandEnv("${HOME}/.rush/rush.yaml"),
		"/etc/rush/rush.yaml",
	} {
