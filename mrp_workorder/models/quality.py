...
class QualityCheck(models.Model):
    ...
    def create(self, value):
        points = self.env['sa.quality.point'].search([
            ('id', '=', value.get('point_id')),
            ('component_id', '!=', False)
        ])
    ...
    @api.depends('point_id', 'quality_state', 'component_id', 'lot_id', 'qty_done')
    ...
    def _get_check_result(self):
        if self.test_type in ('register_consumed_materials', 'register_byproducts') and self.lot_id:
            ...
            return '{}, {}'.format(self.component_id.name, self.qty_done)
        else:
            return ''
