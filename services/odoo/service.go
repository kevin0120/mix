...
func (s *Service) ResultPush(body interface{}) error {
	for _, endpoint := range s.endpoints {
		err := s.resultPush(body, endpoint)
		if err == nil {
			// 如果第一次就成功，推出循环
			break
		}
...
func (s *Service) resultPush(body interface{},endpoint *Endpoint ) error {
	...
	case "PATCH":
		...
		if err != nil {
			return fmt.Errorf("ResultPush fail : %s", err.Error())
		}
	case "PUT":
	...
	default:
		return errors.New("Result push:the Method is wrong ")
