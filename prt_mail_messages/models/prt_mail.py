...
# -- Override Search
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        # Works for mail message only!
        if not self._name == 'mail.message':
        ...
        for msg_model in self._cr.fetchall():
            ...
            # Check if model inherits mail.thread
            parent_names = list(self.env[model]._inherits)

            if 'mail.thread' not in parent_names:
                continue
            if not self.env['ir.model.access'].check(model, 'read', raise_exception=False):
