    ...
    def _push_mrp_routing_workcenter(self, url):
        ...
        for tightening_step_id in tightening_step_ids:
            ...
            for point in operation_point_ids:
                # 拧紧点的拧紧工具序列号没有找到
                if not point.tool_id or not point.tool_id.serial_no:
                    msg = "Can Not Found Tightening Tool For Operation Point :{0}".format(point.name)
                    _logger.error(msg)
                    raise ValidationError(msg)
                if not point.program_id:
                    msg = "Can Not Found Pset For Operation Point :{0}".format(point.name)
                    _logger.error(msg)
                    raise ValidationError(msg)
                _points.append({
                    ...
                    'tightening_tool': point.tool_id.serial_no or '',
                    # 'controller_sn': '',
                    'pset': point.program_id.code if point.program_id else '0',
