...
func TestUriOverride(t *testing.T) {
	...
	l := NewLibrato(ts.URL)
	l.APIUser = "rush@influxdb.com"
	l.APIToken = "123456"
	err := l.Connect()
	require.NoError(t, err)
	err = l.Write([]rush.Metric{newHostMetric(int32(0), "name", "host")})
	require.NoError(t, err)
}
...
func newHostMetric(value interface{}, name, host string) rush.Metric {
	m, _ := metric.New(
		name,
