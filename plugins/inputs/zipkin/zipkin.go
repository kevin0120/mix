...
const (
	...
	// DefaultShutdownTimeout is the max amount of time rush will wait
...
// Zipkin is a rush configuration structure for the zipkin input plugin,
...
// Description is a necessary method implementation from rush.ServiceInput
...
// SampleConfig is a  necessary  method implementation from rush.ServiceInput
...
func (z *Zipkin) Gather(acc rush.Accumulator) error { return nil }

// Start launches a separate goroutine for collecting zipkin client http requests,
// passing in a rush.Accumulator such that data can be collected.
func (z *Zipkin) Start(acc rush.Accumulator) error {
...
func (z *Zipkin) Listen(ln net.Listener, acc rush.Accumulator) {
	if err := z.server.Serve(ln); err != nil {
		...
		// This interferes with rush's internal data collection,
...
func init() {
	inputs.Add("zipkin", func() rush.Input {
		return &Zipkin{
			Path: DefaultRoute,
			Port: DefaultPort,
		}
