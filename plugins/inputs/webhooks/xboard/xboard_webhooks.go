...
type XboardWebhook struct {
	Path   string
	Secret string
	acc    rush.Accumulator
}

func (gh *XboardWebhook) Register(router *mux.Router, acc rush.Accumulator) {
	router.HandleFunc(gh.Path, gh.eventHandler).Methods("POST")
	log.Printf("I! Started the webhooks_github on %s\n", gh.Path)
	gh.acc = acc
}

func (gh *XboardWebhook) eventHandler(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
