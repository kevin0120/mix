...
func (h *Host) SNMPGet(acc rush.Accumulator, initNode Node) error {
	// Get snmp client
	snmpClient, err := h.GetSNMPClient()
	if err != nil {
		return err
...
func (h *Host) SNMPBulk(acc rush.Accumulator, initNode Node) error {
	// Get snmp client
	snmpClient, err := h.GetSNMPClient()
	if err != nil {
		return err
...
func init() {
	inputs.Add("snmp_legacy", func() rush.Input {
		return &Snmp{}
	})
}
