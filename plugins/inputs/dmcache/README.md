...
This plugin requires sudo, that is why you should setup and be sure that the rush is able to execute sudo without a password.

`sudo /sbin/dmsetup status --target cache` is the full command that rush will run for debugging purposes.
...
```
$ ./rush --test --config /etc/rush/rush.conf --input-filter dmcache
