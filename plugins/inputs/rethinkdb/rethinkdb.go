...
func (r *RethinkDB) Gather(acc rush.Accumulator) error {
	if len(r.Servers) == 0 {
...
func (r *RethinkDB) gatherServer(server *Server, acc rush.Accumulator) error {
	var err error
...
func init() {
	inputs.Add("rethinkdb", func() rush.Input {
		return &RethinkDB{}
	})
}
