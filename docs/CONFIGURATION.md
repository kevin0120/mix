...
[rush.conf](https://github.com/masami10/rush/blob/master/etc/rush.conf)

## Generating a Configuration File

A default Rush config file can be auto-generated by rush:
...
```
rush --input-filter cpu:mem:net:swap --output-filter influxdb:kafka config
```
...
`.conf` in the specified directory will also be included in the Rush
configuration.

On most systems, the default locations are `/etc/rush/rush.conf` for
the main configuration file and `/etc/rush/rush.d` for the directory of
configuration files.
...
Rush has a few options you can configure under the `[agent]` section of the
config.

* **interval**: Default data collection interval for all inputs
* **round_interval**: Rounds collection interval to 'interval'
ie, if interval="10s" then always collect on :00, :10, :20, etc.
* **metric_batch_size**: Rush will send metrics to output in batch of at
most metric_batch_size metrics.
* **metric_buffer_limit**: Rush will cache metric_buffer_limit metrics
...
* **debug**: Run rush in debug mode.
* **quiet**: Run rush in quiet mode (error messages only).
* **hostname**: Override default hostname, if empty use os.Hostname().
* **omit_hostname**: If true, do no set the "host" tag in the rush agent.
...
[[outputs.influxdb]]
  urls = [ "http://localhost:8086" ]
  database = "rush-aerospike-data"
  # Only accept aerospike data:
  namepass = ["aerospike*"]

[[outputs.influxdb]]
  urls = [ "http://localhost:8086" ]
  database = "rush-cpu0-data"
