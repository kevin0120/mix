...
class ReportTightResultReport(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        holidays_report = Report._get_report_from_name('tangche_enhanced.tangche_report_result_tighting')
        ...
        query = '''select tb.vin, json_agg(json_build_array(tb.control_date, tb.code, tb.measure_result))
                from (select ors.track_no as vin, ors.control_date as control_date, ors.measure_result as measure_result, cps.code as code from operation_result ors join controller_program cps on cps.id = ors.program_id where ors.control_date >= '%s' and ors.control_date <= '%s' order by ors.control_date) tb
                group by tb.vin''' % (
            date_from,
            date_to
        )
