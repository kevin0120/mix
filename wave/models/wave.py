...
class Wave(models.TransientModel):
    ...
    def _get_echart_data(self,datas,ret):
        ...
        for index, data in enumerate(datas):
            line.add(data['name'], ret['cur_w'].values, ret[u'tor_{0}'.format(data['name'])].values, is_smooth=True, is_more_utils=True)
    ...
    def _get_data(self):
        ...
        ret = None
        _datas = map(lambda x: _create_wave_result_dict(x, client.get_object(bucket, x).data.decode('utf-8')), objects)
