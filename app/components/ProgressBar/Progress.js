...
class ProgressBar extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { shouldCounterReady, shouldCounterStart, shouldCounterPause, shouldCounterStop } = nextProps;
    console.log('will recieve props');
    if (shouldCounterReady && shouldCounterReady() && this.state.counterState !== this.counterStates.ready
      && this.state.counterState !== this.counterStates.start) {
      this.readyCounter();
    }
    if (shouldCounterStart && shouldCounterStart() && this.state.counterState !== this.counterStates.ticking) {
      console.log('should start');
      this.startCounter();
    }
    if (shouldCounterPause && shouldCounterPause() && this.state.counterState === this.counterStates.ticking) {
      this.pauseCounter();
    }
    if (shouldCounterStop && shouldCounterStop() && this.state.counterState !== this.counterStates.stop) {
      this.stopCounter();
    }
