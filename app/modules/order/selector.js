...
import type { tOrderState, tOrderStepIdx } from './interface/typeDef';
import type { IWorkStep } from '../step/interface/IWorkStep';
import type { tAnyStatus } from '../step/interface/typeDef';
import type { IOrder } from './interface/IOrder';

export const workingOrder = (orderState: tOrderState): ?IOrder =>
  orderState?.workingOrder;
export const viewingOrder = (orderState: tOrderState): ?IOrder =>
  orderState?.viewingOrder;
export const viewingIndex = (orderState: tOrderState): tOrderStepIdx =>
  orderState?.viewingIndex;
export const viewingStep = (orderState: tOrderState): ?IWorkStep =>
  orderSteps(viewingOrder(orderState))?.[viewingIndex(orderState)] || null;

export const workingIndex = (order: ?IOrder): tOrderStepIdx =>
  order?.workingIndex || 0;
export const workingStep = (order: ?IOrder): ?IWorkStep =>
  orderSteps(order)?.[workingIndex(order)] || null;

export const orderSteps = (order: ?IOrder): ?Array<IWorkStep> =>
  // eslint-disable-next-line flowtype/no-weak-types
  ((order?.steps: any): Array<IWorkStep>) || null;
export const orderLength = (order: ?IOrder): number =>
  orderSteps(order)?.length || 0;

export const todoOrders = (orderList: Array<IOrder>): Array<IOrder> =>
  orderList &&
  orderList.filter(o => o?.status === ORDER_STATUS.TODO || (o && !o?.status));
