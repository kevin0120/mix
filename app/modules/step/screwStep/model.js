...
class ClsOperationPoint {
  ...
  isFinalFail(): boolean {
    ...
    return rsCount >= this._point.maxRetryTimes && this._results[rsCount - 1] === RESULT_STATUS.nok;
  }
...
class ClsOperationPointGroup {
  ...
  appendNewOperationPoint(p: tPoint): boolean {
    if(!this.validatePoint(p)){
      CommonLog.lError(`appendNewOperationPoint validatePoint Error: ${p.toolSN}`);
      return false;
    }
    this._operationPoints.push(new ClsOperationPoint(p));
    this._operationPoints.sort((a: ClsOperationPoint, b: ClsOperationPoint) => a.point.sequence - b.point.sequence); // 排序操作
    return true;
  }
  ...
  _appendNewOperationPoint(p: tPoint) {
    ...
    switch (isExist) {
      case true:
        ...
        break;
      case false:
        CommonLog.Debug(`Group Seq: ${groupSeq} Is Not Exist! We Will Create It`);
        pg = new ClsOperationPointGroup(groupSeq);
        this._operationGroups[groupSeq] = pg;
        break;
      default:
        CommonLog.lError("tOrderOperationPoints _appendNewOperationPoint Error!")
    }
    if (pg) {
