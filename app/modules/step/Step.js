...
import type { tStep } from './interface/typeDef';
import { STEP_STATUS, stepTypeKeys } from './constants';

const StepMixin = (ClsWorkable: Class<IWorkable>) => class Step extends ClsWorkable implements IWorkStep {
  ...
  update(stepData: ?$Shape<tStep>) {
    ...
    const { sequence, test_type: testType, failure_msg: failureMsg, desc, image, skippable, undoable, data, status } = stepData || {};
    this._sequence = sequence;
    this._type = testType;
    this._image = image;
    this._skippable = skippable;
    this._undoable = undoable;
    (this: IWorkable)._status = status || (this: IWorkable)._status;
