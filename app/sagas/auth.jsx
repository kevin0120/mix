...
function* authorize(user, password) {
  ...
  const isExisted = lodash.some(users, {uuid: u}) || lodash.some(users, {name: u}); // 检测是否已经登录
  if (isExisted)
    return;
  try {
    const response = yield call(getUserInfo, setting.page.odooConnection.odooUrl.value, u, password);
    const statusCode = response.status;
    if (statusCode === 200){
      const { id, name, uuid, image_small} = response.data;
      yield put({type: USER.LOGIN_SUCCESS, uid: id, name, uuid, avatar: image_small });
    }
  }catch (e) {
